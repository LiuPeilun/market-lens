...
def compact_analysis_for_llm(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "valuation": {
            ...
            "index_data_route": valuation.get("index_data_route"),
    ...
    return [
        {
            ...
            "content": (
                ...
                "如果数据不足，要明确说明缺口。"
                "assessment.status=unavailable 时，必须明确说明当前不能形成估值，"
                "不得输出估值分、估值等级、收益表现或投资结论；"
                "应依据 fallback_reasons 和 data_quality 中的稳定诊断说明原因；"
                "存在 chat_tool_failure 时，只能依据其中的稳定分类说明工具失败和是否可重试。"
